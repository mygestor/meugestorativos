...
export function FIIAnalysis({ fiiAssets, hideValues, onEdit, onRefresh }: Props) {
  ...
  const rows = useMemo(() => {
    return localAssets.map((a) => {
      // Dividend records ordenados do mais recente
      const allDivs = allDividends
        .filter((d) => d.ticker.toUpperCase() === a.ticker.toUpperCase())
  ...
  const filtered = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case "dy_m": av = a.divYieldMensal; bv = b.divYieldMensal; break;
        case "dy_ano": av = a.dyB3 > 0 ? a.dyB3 : a.divYieldMensal * 12; bv = b.dyB3 > 0 ? b.dyB3 : b.divYieldMensal * 12; break;
          ...
          <div className="bg-card border border-border rounded-2xl p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              ...
              <MiniBox label="DY Mensal" value={formatPercent(r.divYieldMensal)} color={yieldColor(r.divYieldMensal)} />
               {r.dyB3 > 0 && <MiniBox label="DY 12M (com JCP)" value={formatPercent(r.dyB3)} color={yieldColor(r.dyB3 / 12)} />}
