      ...
      <div className="bg-card border border-border rounded-2xl p-8 w-full max-w-sm space-y-6">
        ...
        {showSetup ? (
          <div className="space-y-3">
            <p className="text-xs text-muted">Cole o <code>firebaseConfig</code> do Firebase Console:</p>
            <textarea
              value={configText}
              onChange={(e) => setConfigText(e.target.value)}
              placeholder='{"apiKey":"...","authDomain":"...","projectId":"..."}'
              className="w-full h-32 px-3 py-2 rounded-xl bg-surface border border-border text-xs font-mono resize-none"
            />
            <div className="flex gap-2">
              <button onClick={() => setShowSetup(false)} className="flex-1 px-4 py-2 rounded-xl text-sm bg-surface text-muted">Cancelar</button>
              <button onClick={handleSetupConfig} className="flex-1 px-4 py-2 rounded-xl text-sm bg-primary text-white">Salvar</button>
            </div>
          </div>
        ) : (
          <>
